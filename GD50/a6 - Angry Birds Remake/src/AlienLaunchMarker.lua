...
function AlienLaunchMarker:update(dt)
    ...
    if self.launched and love.keyboard.wasPressed('w') and not self.split and not collided then

        local xPos,yPos = self.alien.body:getPosition()
        local xVel, yVel = self.alien.body:getLinearVelocity()
        self.split = true
        self.alien2 = Alien(self.world,'round',xPos,yPos-40,'player')
        self.alien2 .body:setLinearVelocity(xVel+20,yVel+25)
        self.alien2.fixture:setRestitution(0.4)
        self.alien2.body:setAngularDamping(1)
       

        self.alien3 = Alien(self.world,'round',xPos,yPos+40,'player')
        self.alien3 .body:setLinearVelocity(xVel+20,-3)
